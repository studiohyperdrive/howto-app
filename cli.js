module.exports = (requireProxy, { root }) => {
  /**
   * @license
  ...
  requireProxy("symbol-observable");
  // tslint:disable-next-line:ordered-imports import-groups
  const core_1 = requireProxy("@angular-devkit/core");
  const node_1 = requireProxy("@angular-devkit/core/node");
  const schematics_1 = requireProxy("@angular-devkit/schematics");
  const tools_1 = requireProxy("@angular-devkit/schematics/tools");
  const inquirer = requireProxy("inquirer");
  const minimist = requireProxy("minimist");
  /**
  ...
  async function main({ args, stdout = process.stdout, stderr = process.stderr, }) {
      ...
      const workflow = new tools_1.NodeWorkflow(fsHost, { force, dryRun, registry, root });
